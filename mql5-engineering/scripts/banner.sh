...
STATIC=0
if [[ "${1:-}" == "--static" ]] || [[ "${MQL5_ENGINEERING_NO_ANIMATION:-0}" == "1" ]]; then STATIC=1; fi

supports_ansi=0
if [[ -t 1 ]] && [[ "${TERM:-dumb}" != "dumb" ]]; then supports_ansi=1; fi

if [[ "$supports_ansi" -eq 1 ]] && [[ -z "${NO_COLOR:-}" ]]; then
  GREEN_BRIGHT=$'\033[1;32m'; GREEN=$'\033[0;32m'; GREEN_DIM=$'\033[2;32m'; RESET=$'\033[0m'
else
  GREEN_BRIGHT=""; GREEN=""; GREEN_DIM=""; RESET=""
fi

cleanup(){ if [[ "$supports_ansi" -eq 1 ]]; then printf '\033[?25h'; fi; }
...
print_banner(){
  ...
  for line in "${hero_lines[@]}"; do
    ...
    if [[ "$STATIC" -ne 1 ]] && [[ "$supports_ansi" -eq 1 ]]; then sleep 0.014; fi
  done
...
loading_animation(){
  ...
  local frames=('Carregando   ' 'Carregando.  ' 'Carregando.. ' 'Carregando...'); local i
  for i in 0 1 2 3 0 1 2 3; do
    printf '\r\033[2K%s%s%s' "$GREEN" "${frames[$i]}" "$RESET"; sleep 0.16
  done
  printf '\r\033[2K%sPronto para uso!%s\n\n' "$GREEN_BRIGHT" "$RESET"
}

if [[ "$supports_ansi" -eq 1 ]] && [[ "$STATIC" -ne 1 ]]; then printf '\033[?25l'; fi
print_banner
loading_animation
